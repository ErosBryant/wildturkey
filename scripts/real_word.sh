...
output_dir="/mnt/wildturkey/experiment/real_word_workload/"
   ...
   # for lac in "${lacd[@]}"; do
      ...
      for md in "${mod[@]}"; do
         ...
         for i in $(seq 1 $number_of_runs); do
               ...
               # f=$((max / 2)) 
               # --lsize=$f
               ${test_dir}/db_bench --benchmarks="${wkload},real_r,99p,stats" --mod=$md --num=$num >> "$output_file"
               echo "-------------------------------------" >> "$output_file"
